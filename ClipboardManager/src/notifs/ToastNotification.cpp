...
{
    bool ToastNotification::registered = false;
    ...
    void ToastNotification::addText(const std::wstring& text)
    {
    ...
    void ToastNotification::addButton(const std::wstring& content, const std::wstring& tag)
    {
        buttonElements.push_back({ content, tag });
    }

    bool ToastNotification::tryAddButtons(const std::vector<std::pair<std::wstring, std::wstring>>& buttons)
    {
        if (buttons.size() > 5)
        {
            return false;
    ...
    void ToastNotification::send()
    {
        send(NotificationDurationType::Default, NotificationScenarioType::Default, NotificationSoundType::Default);
    }

    void ToastNotification::send(const NotificationDurationType& durationType, const NotificationScenarioType& scenarioType, const NotificationSoundType& soundType)
    {
        if (!registered)
        {
    ...
    <toast>
        ...
        auto toastNotifier = toasts::compat::DesktopNotificationManagerCompat::CreateToastNotifier();
        toastNotifier.Show(notification);
    }


    void ToastNotification::ensureToastManagerRegistered()
    {
        logger.info(L"Ensuring that DesktopNotificationManagerCompat has been registered.");
        try
        {
            toasts::compat::DesktopNotificationManagerCompat::Register(
